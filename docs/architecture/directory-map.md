...
| 路径 | 职责 | 关键类 / 入口 |
| --- | --- | --- |
| `python/sglang/launch_server.py` | 服务启动总入口（CLI `python -m sglang.launch_server`） | `launch_server()` |
| `python/sglang/srt/entrypoints/` | HTTP / gRPC / OpenAI / Anthropic / Ollama 兼容入口、Engine 封装 | `Engine`(`entrypoints/engine.py`)、`HTTPServer`(`http_server.py`) |
| `python/sglang/srt/managers/` | **多进程调度核心**：Tokenizer 管理、Scheduler、Detokenizer | `Scheduler`、`TokenizerManager`、`DetokenizerManager` |
| `python/sglang/srt/mem_cache/` | KV 缓存与 Radix 树、显存分配器 | `RadixCache`、`TokenToKVPoolAllocator`、多级缓存 |
...
| `python/sglang/srt/models/` | 各模型实现（216 个文件） | 如 `llama.py`、`deepseek_v3.py` |
...
| `python/sglang/srt/speculative/` | 投机解码（EAGLE / n-gram 等） | `SpecInfo`、`DraftWorker` |
| `python/sglang/srt/constrained/` | 语法/JSON 约束解码（集成 xgrammar / outlines） | `Constraint` |
| `python/sglang/srt/lora/` | LoRA 适配器加载与合并 | `LoRAManager` |
| `python/sglang/srt/multimodal/` | 多模态输入处理与缓存 | `MultimodalData`、processors |
| `python/sglang/srt/disaggregation/` | PD 分离（prefill/decode 解耦） | `PrefillWorker` / `DecodeWorker`、connector |
| `python/sglang/srt/observability/` | 指标、日志、profiling | `MetricsCollector` |
...
| `python/sglang/lang/` | 前端 DSL 编译器与运行时 | `Program`、`Interpreter` |
...
graph TD
    ...
    subgraph MEM["显存与缓存"]
        ...
        POOL["TokenToKVPoolAllocator<br/>(mem_cache/memory_pool.py)"]
    end
    SCH -.管理.-> MEM
```
